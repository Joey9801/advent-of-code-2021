...
fn parse_input(raw: &str) -> Input {
    ...
    assert!(pairs.iter().flat_map(|k| k.chars().map(|c| c.is_ascii_uppercase())).all(|x| x));
    
    let init_str = raw.lines().next().unwrap();
    let mut init_state = vec![0; pairs.len()];
    for i in 0..(init_str.len() - 1) {
        let pair = pair_map.get(&init_str[i..(i+2)])
    ...
    for op_line in raw.lines().skip(2) {
        ...
        let first_dest = *pair_map.get(&format!("{}{}", first_char, new_char))
            .expect("Expect newly formed pair to be in map");
        let second_dest = *pair_map.get(&format!("{}{}", new_char, second_char))
            .expect("Expect newly formed pair to be in map");
