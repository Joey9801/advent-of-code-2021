...
pub fn parse_input(s: &'static str) -> Map {
...
pub fn part_2(map: &Map) -> i32 {
    ...
    path.push(start);
    to_visit.push(map.edges[start.0]);
    smalls_visited.add(start);
    ...
    while let Some(path_head) = path.last().cloned() {
        ...
        if path_head == end {
            ...
            // Don't consider any extensions to the path past the end node
            path.pop();
            to_visit.pop();
...
pub fn part_2_memo(map: &Map) -> u32 {
